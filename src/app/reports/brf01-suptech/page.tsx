...
import type { Brf01SupTechMetrics } from "@/lib/brf01SupTechTemplate";
import {
...
export default function Brf01SupTechReportPage() {
  const [draftTimeKey, setDraftTimeKey] = useState("");
  const [draftEntityGroups, setDraftEntityGroups] = useState<string[]>([
    BRF01_SUPTECH_ENTITY_GROUPS[0],
  ]);
  const [draftDataSources, setDraftDataSources] = useState<string[]>([
    BRF01_SUPTECH_DATA_SOURCES[0],
  ]);
  ...
  const [entries, setEntries] = useState<Brf01SupTechEntry[]>([]);
  ...
  const loadEntries = useCallback(async () => {
    ...
    const response = await fetch(`/api/brf01-suptech?${params.toString()}`);
    if (response.ok) {
  ...
  function handleDownload() {
    ...
    window.location.href = `/api/brf01-suptech/export?${params.toString()}`;
  }

  return (
    <AppShell active="/reports" title="BRF 1.1 - Assets (Sup Tech)">
        ...
        <div className="rounded-lg border border-zinc-200 bg-white shadow-sm p-5">
          <div className="flex items-start justify-between gap-4">
            <p className="text-sm font-semibold text-black">
              CBUAE Banking Return Form 1.1 — Assets (Sup Tech template)
            </p>
            <button
          ...
          <div className="mt-4 flex flex-wrap items-end gap-6">
            ...
            <MultiSelectDropdown
              label="Entity group"
              options={BRF01_SUPTECH_ENTITY_GROUPS}
            ...
            <MultiSelectDropdown
              label="Data source"
              options={BRF01_SUPTECH_DATA_SOURCES}
            ...
            <div className="max-h-[70vh] overflow-auto">
              <table className="w-full border-collapse text-left text-sm text-zinc-900">
                <thead className="sticky top-0 z-10">
                  <tr>
                    <th rowSpan={4} className="border border-sky-400 bg-sky-300 px-2 py-2 align-bottom font-semibold text-sky-950">Row No</th>
                    <th rowSpan={4} className="border border-sky-400 bg-sky-300 px-2 py-2 align-bottom font-semibold text-sky-950">Line No</th>
                    <th rowSpan={4} className="border border-sky-400 bg-sky-300 px-2 py-2 align-bottom font-semibold text-sky-950">Description</th>
                  ...
                  <tr className="text-[11px] italic text-sky-950">
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0010</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0020</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0030</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0040</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0050</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0060</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0070</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0080</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0090</th>
                    <th className="border border-sky-400 bg-sky-300 px-2 py-0.5 font-normal">0100</th>
                  </tr>
                </thead>
                <tbody>
                  {entries.map((entry) => (
                    ...
                    >
                      <td className="border border-zinc-200 px-2 py-1">{entry.rowNo}</td>
