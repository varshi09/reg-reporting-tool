...
import {
  BRF01_SUPTECH_TABLE,
  BRF01_SUPTECH_TEMPLATE,
  BRF01_SUPTECH_COLUMNS,
  EMPTY_SUPTECH_METRICS,
  type Brf01SupTechMetrics,
...
export async function GET(request: Request) {
  ...
  const byCode = new Map<string, Brf01SupTechMetrics>();
  ...
  const sheet = workbook.addWorksheet("BRF 1.1 - Assets (Sup Tech)");
  ...
  const { metricKeys } = writeBrfMetricHeader(
    sheet,
    headerStartRow,
    [
      { header: "Row No", width: 10 },
      { header: "Line No", width: 10 },
      { header: "Description", width: 55 },
    ],
    { columnNoValues: BRF01_SUPTECH_COLUMNS.map((col) => col.colNo) }
  );
  ...
  for (const templateRow of BRF01_SUPTECH_TEMPLATE) {
    const metrics = byCode.get(templateRow.code) ?? EMPTY_SUPTECH_METRICS;
    const row = sheet.addRow({
      rowNo: templateRow.rowNo,
  ...
  return new Response(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="BRF1_1_Assets_SupTech_${
        timeKey ?? "all"
      }.xlsx"`,
    },
  });
