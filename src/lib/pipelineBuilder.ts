...
export async function getPipelineStructure(pipelineId: number): Promise<PipelineStructure | null> {
  type PipelineRow = { ID: number; NAME: string };
  type GroupRow = {
  ...
  return withConnection(async (connection) => {
    ...
    return { pipelineId: pipeline.ID, pipelineName: pipeline.NAME, groups };
  });
}
...
export async function deleteGroup(groupId: number, pipelineId: number): Promise<void> {
  await withConnection(async (connection) => {
    // Detach procedures that belong to this group (set group_id null rather than delete the PP row)
